---
title: "Informe Práctica - GRUPO C"
author: "Fabi Páez - Moris Davitashvili - Pedro Galindo"
...
### Obtención de datos
Los datos utilizados para el análisis han sido extraídos de la API de Twitter. La API de Twitter dispone de múltiples métodos para extraer información de los tweets de los usuarios. En nuestro caso, hemos usado la función search() para buscar los tweets relacionados con diferentes ataques cibernéticos  correspondientes a los años 2015, 2016 y 2017.

Esta búsqueda ha sido limitada al no disponer de una cuenta premium del servicio. Además, a lo largo del proceso nos hemos dado cuenta de ciertas limitaciones que han condicionado los resultados. La API gratuita solo dispone de los tweets de los últimos días. Por otro lado, debido al poco tiempo para realizar la práctica, hemos tenido que limitar la cantidad de información al tardar mucho los procesos de obtención de gráficos implementados por la gran cantidad de datos.
...
+	API Twitter - Accedemos a Twitter con la función setup_twitter_oauth mediante claves generadas en el proceso de registro de un nuevo usuario.
...
+ Realizamos un bucle mediante la sentencia for, con el objetivo de realizar varias búsquedas y guardar esta información de manera ordenada.
+ Como resultado de la ejecución de este fragmento de código obtenemos un documento con nombre "nombreAtaque.rds". La extensión .rds nos permite guardar y posteriormente recuperar el dataFrame tal y como se había guardado.
...
+ Buscamos información sobre los usuarios.
+ Convertimos esta nueva información a DataFrame.
+ Guardamos la información relativa a la localitzación de los usuarios.
...
+ Empezamos con el proceso de limpieza de los países. Primero eliminando los NA.
+ Seguidamente nos quedamos con los países.
...
+ Creamos un gráfico circular que representa como se ha dividido el número de tweets en los países.
...
+ Como resultado tenemos un grafico con el numero de tweets de cada día.
...
+ Guardamos la informacion relativa a la localitzación de los usuarios.
...
+ Conservamos la relación de altura / anchura (proporciones).
+ Quitamos información no útil del fondo.
+ Como resultado pintamos las coordenadas correspondientes a la ubicacion de los tweets sobre un mapamundi.
...
+ Recuperamos el dataFrame guardado en el fichero .rds.
+ Recogemos el tweet (el texto del tweet).
+ Limpiamos los tweets quitando comas, números, link html, espacios, caracteres raros (@, ^, \\, :, etc.).
+ Dividimos la frase en palabras sueltas.
...
+ Contamos el número de línias de cada fichero .rds con el objetivo de determinar el número de tweets de cada ataque.
+ Creamos una matriz que contiene por una parte el número de tweets y por otra el nombre del ataque.
+ Como resultado obtenemos un grafico que permite comparar el número de tweets producidos entre varios ataques.
...
### 3.1.1 DDoS Playstation mapamundi

En este segundo gráfico, vemos los diferentes tweets pintados en el mapa mundi:

<!-- mapamundi Playstation -->
![No se carga la imagen](worldPlaystation.png)



### 3.1.2 DDoS Playstation apple

En este gráfico, podemos observar la proporción de tweets por los diferentes países de origen:

<!-- llamada a las funciones  -->
![No se carga la imagen](piePlaysation.png)

### 3.1.3 DDoS Playstation palabras repetidas
...
```{r , echo=FALSE, comment=" ", echo=FALSE, warning=FALSE, message=FALSE, results=FALSE}
...
<!-- grafico Wannacry-->
![No se carga la imagen](tempsTweetsWanna.png)
...
En este segundo gráfico, vemos los diferentes tweets pintados en el mapa mundi:
<!-- mapamundi Wannacry-->
![No se carga la imagen](mapaWanna.png)

### 3.2.3 Wannacry apple
En este gráfico, podemos observar la proporción de tweets por los diferentes países de origen:
<!-- apple Wannacry -->
![No se carga la imagen](pieWanna.png)
...
![No se carga la imagen](wordCloudWanna.png)
...
En este segundo gráfico, vemos los diferentes tweets pintados en el mapa mundi:
![No se carga la imagen](petyaWorldMap.png)
...
Como se muestra a continuación, la siguiente gráfica compara el numero de tweets de los diferentes ataques analizados:
