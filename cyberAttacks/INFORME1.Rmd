...
### Datos accesibles y obtenidos FLOJO!! MODIFICAR

Para poder obtener la información y así poder responder a la pregunta formulada se ha utilizado el API de twitter con el objetivo de así poder utilizar sus metodos con la finalidad de obtener la informació para posteriormete procesarla.

El proyecto desarrollado en el marco de este trabajo tiene como objetivo detectar .. MODIFICAR ...
...
+ Como resultado de la ejecución de este fragmento de codigo obtenemos un documento con nombre "nombreAtaque.rds". La extensión .rds nos permite guardar y posteriormente recuperar el dataFrame tal y como se habia guardado.
...
```{r r_sample1, echo=FALSE}
  ...
  paintTweetsDays("playstation.rds")
```
...
```{r r_sample2, echo=FALSE}
  library(cyberAttacks)
  wordCloudTweets("playstation.rds")
```
...
<!-- grafico Wannacry -->
```{r r_sample5, echo=FALSE}
  library(cyberAttacks)
  paintTweetsDays("wannacry.rds")
```
...
```{r r_sample6, echo=FALSE}
  library(cyberAttacks)
  paintWorldMap("wannacry.rds")
```
 -->
...
```{r r_sample7, echo=FALSE}
  library(cyberAttacks)
  paintContries("wannacry.rds")
```

### 3.2.4 Wannacry palabras repetidas

<!-- palabras repetidas Wannacry -->
```{r r_sample8, echo=FALSE}
  library(cyberAttacks)
  wordCloudTweets("wannacry.rds")
```
...
```{r r_sample15, echo=FALSE}
  library(cyberAttacks)
  paintContries("keyrayder.rds")
```

### 3.4.4 KeyRaider palabras repetidas

<!-- palabras repetidas KeyRaider -->
```{r r_sample16, echo=FALSE}
  library(cyberAttacks)
  wordCloudTweets("keyrayder.rds")
```
...
```{r r_sample21, echo=FALSE}
  library(cyberAttacks)
  wordCloudTweets("wannacry.rds","playstation.rds","petya.rds","keyrayder.rds","judy.rds")
```
